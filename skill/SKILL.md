...
5. **Link**: semantic forward-linking (LLM, phases 1–2) using basename `[[wikilinks]]`, then run `scripts/repair_vault.py --vault ${VAULT_PATH} --project ${PROJECT_TAG} --links-only` (phase 3) to deterministically symmetrize every link and materialize ≥3-ref stubs. This makes the validation gate see zero asymmetric links — no per-run repair churn.
...
11. **Never create `status: draft` stubs for one-off dangling wikilinks** (< 3 referrers) — leave them as "create new" anchors. The ≥3-referrer stub materialization (for `node_` AND `entity_`) is done **deterministically** by `repair_vault.py --links-only`, not by LLM judgment. See `references/multi-agent-orchestration.md` → linker phase 3.
12. **Link bidirectionality and ≥3-ref stubs are mechanical, not LLM work.** The linker does semantic forward-linking (phases 1–2) then MUST run `repair_vault.py --vault ${VAULT_PATH} --links-only` (phase 3) to symmetrize every link and materialize ≥3-ref stubs. Asymmetric links reaching the validation gate means phase 3 was skipped — that is a linker failure. An isolated `node_*` (`linked_nodes: []`) is also a linker failure.
