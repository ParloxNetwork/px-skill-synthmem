...
| `/synthmem repair` | Reconcile an existing vault: deterministically fixes legacy issues the validator detects (tag distinctness in meta/archive/log, slug==filename-tail, missing reverse wikilinks, archive content-type, render-breaking `<…>`). Never deletes content. The fix for vaults made by older skill versions. Run it when the gate reports pre-existing errors. |
