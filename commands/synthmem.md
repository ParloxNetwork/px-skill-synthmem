...
| `/synthmem status` | Read-only operational dashboard: vault size + file counts, last-run timestamp/status, total processed + streak, draft-stub count, pending sessions, days since last run. No validation, no writes. Fast on large vaults. |
| `/synthmem --retry` | Reprocess **only** the failed sessions in `pending_sessions` (not the whole window). Auto-drops a session after 3 failed attempts (into `dropped_sessions`, reported). Normal runs already auto-resume pending work; this is the explicit "clear failures now" form. |
| `/synthmem validate` | Read-only. Runs the deterministic vault validator (frontmatter, slugs, tags, wikilinks, duplicates, markdown safety, tag-genericity advisory) and prints PASS/REVIEW/FAIL. No consolidation, no writes. Cheap way to audit a large vault. |
